...
This foundation uses a plugin-first desktop shell. The `MainWindow` owns only navigation and workspace composition; feature modules are discovered by the plugin loader and rendered through the `Plugin` interface.

## Boundaries

- `app`: application composition and process entry point.
- `core`: configuration, logging, resources, theming, exceptions, context, and dependency injection.
