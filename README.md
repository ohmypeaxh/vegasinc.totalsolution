...
- 적격성평가 종류와 문서번호는 각각 `##적격성종류##`, `##문서번호##`에 삽입됩니다. IOQ 명칭은 `INSTALLATION & OPERATIONAL QUALIFICATION`을 사용합니다. 회사 로고는 `##로고##` 위치에 높이 0.95cm, 원본 비율 유지로 삽입됩니다.
...
1. `개별표지`, `개별라벨` 시트와 12개 자리표시자가 있는 XLSX 템플릿을 선택합니다.
2. 영문 장비명, 적격성평가축약, 계획서번호, 보고서번호, 해당년도를 입력하고 고객사 로고를 드래그앤드롭합니다.
3. 적격성평가축약 선택에 따라 한글·영문 명칭이 자동으로 치환됩니다.
4. `##계획서번호##`, `##보고서번호##`를 각각 입력값으로 치환하고, 고객사 로고는 `##고객사로고1##`~`##고객사로고5##` 위치에 원본 비율과 지정 크기를 유지해 가운데 정렬됩니다. 로고 4·5의 너비는 3.58cm입니다.
5. 저장 폴더를 선택하고 `Cover PDF 생성`을 누르면 `장비명_적격성평가축약_cover.pdf` 이름의 정확히 2페이지 PDF가 생성됩니다.
6. `Print`를 누르면 저장 폴더에 PDF를 남기지 않고 임시로 Cover를 준비한 뒤 Windows 인쇄 창을 바로 엽니다. 인쇄 완료 또는 취소 후 임시 파일은 삭제됩니다.
...
GitHub에서 **Actions → Build Lightweight Windows Update** 실행 결과를 열고 `Vegas-Total-Solution-Doc-Update-0.1.5` artifact를 내려받습니다. 기본 설치본이 없거나 Python·PySide6 같은 런타임 의존성이 바뀐 버전에는 전체 설치파일을 사용해야 합니다.
